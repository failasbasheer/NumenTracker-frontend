...
export default function TransactionList() {
  ...
  const typeLabelClass = (type: "income" | "expense") =>
    `text-xs font-medium px-2 py-0.5 rounded ${
      type === "income" ? "bg-emerald-950 text-emerald-400" : "bg-red-950 text-red-400"
    }`

  const amountClass = (type: "income" | "expense") =>
    `font-semibold text-base ${type === "income" ? "text-emerald-400" : "text-red-400"}`
            ...
            <AnimatePresence>
              {data.slice(0, 10).map((tx) => (
                <motion.li
                  ...
                  exit="exit"
                  layout
                  className="flex items-center justify-between px-5 py-3 hover:bg-gray-900/50 transition-transform duration-150 rounded-md cursor-pointer hover:scale-[1.02] shadow-sm"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-gray-100 text-sm truncate">{tx.category}</span>
                      <span className={typeLabelClass(tx.type)}>{tx.type}</span>
                    </div>
                    <div className="text-xs text-gray-500">
                      {new Date(tx.date).toLocaleDateString("en-IN", {
                        month: "short",
